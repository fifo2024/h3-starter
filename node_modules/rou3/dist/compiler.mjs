...
function compileRouteMatch(router, deps, opts) {
	...
	if (match) str += "let s=p.split('/'),l=s.length-1;" + match;
	if (!str) return opts?.matchAll ? "return [];" : "";
...
function compileNode(node, params, startIdx, deps, isParamNode, staticNodes, opts) {
	...
	if (node.methods && !staticNodes.has(node)) {
		const match = compileMethodMatch(node.methods, params, deps, isParamNode ? startIdx : -1, opts);
		if (match) str += `if(l===${startIdx}${isParamNode ? `||l===${startIdx - 1}` : ""}){${match}}`;
	}
	if (node.static) for (const key in node.static) {
		const match = compileNode(node.static[key], params, startIdx + 1, deps, false, staticNodes, opts);
		if (match) str += `if(s[${startIdx + 1}]===${JSON.stringify(key)}){${match}}`;
	}
	if (node.param) {
		const match = compileNode(node.param, [...params, `s[${startIdx + 1}]`], startIdx + 1, deps, true, staticNodes, opts);
		if (match) str += match;
	}
	if (node.wildcard) {
		const { wildcard } = node;
		...
		if (wildcard.methods) {
			const match = compileMethodMatch(wildcard.methods, [...params, `s.slice(${startIdx + 1}).join('/')`], deps, startIdx, opts);
			if (match) str += match;
		}
	}
	return str;
